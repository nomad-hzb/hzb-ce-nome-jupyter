import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import time 
from datetime import datetime

from api_calls import *

def print_cp_overview(data):
    print('Number of CP files in selected upload:', len(data))
    if data:
        print('Timestamp first CP file:', data[0]["datetime"])

def print_overview(data, entry_type):
    print(f'Number of {entry_type} files in selected upload: {len(data)}')
    if data:
        print(f'Timestamp first {entry_type} file: {data[0]["datetime"]}')
    
def get_cp_overview_dataframe(data):
    data.sort(key=lambda x: datetime.strptime(x.get('datetime'), "%Y-%m-%dT%H:%M:%S+%f:00"))
    time = []
    voltage = []
    current = []
    current_density = []
    for d in data:
        expected_length = len(d.get('time', []))
        voltage.extend(d.get('voltage', [None]*expected_length))
        current.extend(d.get('current', [None]*expected_length))
        current_density.extend(d.get('current_density', [None]*expected_length))
        time_tmp = np.array(d.get('time', []), dtype=np.float64)
        time_tmp += np.max(time) if time else 0
        time.extend(time_tmp)  
    df = pd.DataFrame(data={'time': time, 'voltage': voltage, 'current': current, 'current_density': current_density,},)
    return df

def plot_overview(df, entry_type):
    plt.close(1)
    plt.figure(1)
    plt.plot(np.array(df.time)/3600, df.voltage, linestyle='solid', color='darkblue',)
    #label="$E_{\mathrm{applied}} = 250\, \mathrm{mA\, cm^{-2}}$",)
    plt.title(entry_type, fontsize=18)
    plt.xlabel('Time [hr]', loc='center', fontsize=14)
    plt.ylabel('Voltage [V]', loc='center', fontsize=14)
    plt.tick_params(axis='both', which='major', labelsize=12)
    plt.show()

def plot_interval(df, entry_type = 'CE_NOME_Chronopotentiometry'):
    plt.close(2)
    plt.figure(2)
    plt.plot(np.array(df.time), df.voltage, linestyle='solid', color='darkgreen',)
    plt.title(entry_type, fontsize=18)
    plt.xlabel('Time [s]', loc='center', fontsize=14)
    plt.ylabel('Voltage [V]', loc='center', fontsize=14)
    plt.tick_params(axis='both', which='major', labelsize=12)
    plt.show()

def run_analysis(url, token, upload_name, entry_type = 'CE_NOME_Chronopotentiometry'):
    data = get_specific_entrytype_of_uploads(url, token, [upload_name], entry_type, with_meta=False)
    print_overview(data, entry_type)
    df = get_cp_overview_dataframe(data)
    if df.empty:
        print(f'Please select another upload that contains entries of type {entry_type}.')
    else:
        plot_overview(df, entry_type)
    return df

